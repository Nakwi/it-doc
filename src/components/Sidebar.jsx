...
export default function Sidebar({ currentPath = '', procedureCounts = {} }) {
  ...
  const getTotalProcedures = (categoryId) => {
    const categoryCounts = procedureCounts[categoryId] || {};
            ...
            <div key={category.id}>
              <button
                ...
                className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                  currentPath.startsWith(`/${category.id}`)
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs text-[var(--color-text-secondary)]">
                    {getTotalProcedures(category.id)}
                  </span>
                  <svg
                    className={`w-4 h-4 transition-transform ${expandedCategories[category.id] ? 'rotate-90' : ''}`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                ...
                <div className="ml-4 mt-1 space-y-1 border-l border-[var(--color-border)] pl-3">
                  {category.subcategories
                    ...
                    >
                      ...
                      <span className="text-xs text-[var(--color-text-secondary)]">
                        {getSubcategoryCount(category.id, subcategory.id)}
                      </span>
                    </a>
                  ))}
                </div>
